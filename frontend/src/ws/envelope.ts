...
export const FRAME_TABLE: Record<WsFrameType, FrameSpec> = {
  telemetry: {
    ...
    // Mirrors `contracts/ws/schema.py`'s TELEMETRY row. Consumers read different
    // halves — `S-03` reads `motor_states`, `S-04` and the viewport read `joints`,
    // the policy path reads `observation` — so all of them travel.
    fields: ["sequence", "observation", "motor_states", "arms", "joints"],
  },
  command: {
    frameType: "command",
