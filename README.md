...
> 🔴 **[`docs/v1/STATUS.md`](docs/v1/STATUS.md) 를 먼저 읽어라.** 명세와 계획은 *하려던 것*이고 STATUS 는 *지금 도는 것*이다. 이 저장소에는 파일이 있고 테스트가 초록인데 **아무 실행 경로에도 안 걸린** 코드가 많다 — **진입점에서 도달하는 것은 29%뿐이다**(측정 방법은 STATUS §2). 그 구분 없이 트리 이름만 보면 없는 기능을 있다고 읽는다.
