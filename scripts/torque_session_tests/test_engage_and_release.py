...
class _Bench:
    ...
    def __init__(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, manifest: bool = True
    ) -> None:
        ...
        def make_writer(slots: tuple[Any, ...]) -> FanoutWriter:
...
# --- The refusal on this host: the write path is half assembled ---


def test_both_torque_steps_refuse_while_no_single_writer_exists(tmp_path: Path) -> None:
    """The rig binding is not a write path on its own, and half a write path engages nothing.

    Not skipped, asserted: this is the state the runner is actually in, and the two steps have
...
def test_the_engage_frame_carries_the_left_arm_angles_to_the_left_channel(
    ...
    first = FITTED[0]
    assert bench.buses[SIDE_LEFT].sent[-1][first][2] == pytest.approx(LEFT_BASE_DEG)
    assert bench.buses[SIDE_RIGHT].sent[-1][first][2] == pytest.approx(RIGHT_BASE_DEG)
    _release(bench)
...
def test_the_release_drops_the_fitted_motors_and_stops_the_re_send(
    ...
    assert bench.writers[-1].write_count == len(bench.writers[-1].batches)
