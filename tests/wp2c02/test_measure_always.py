...
run at, which is what lets the effective delay be shown the moment PG-FRIC-001 passes. Adding the
observer gain as a second required input did not make the band skippable, which is what the
DISABLED cases here still prove.
"""
...
def test_disabled_verdict_still_carries_a_measured_band(
    deferred_status: str, fmax_deferred, frames: int, observer_gain: float
) -> None:
    """A locked (DISABLED) verdict still carries the measured band — never skipped."""
    activation = measure_and_resolve(deferred_status, frames, fmax_deferred, observer_gain)
...
def test_effective_latency_is_reciprocal_of_rate(
...
def test_band_provenance_is_carried(
    synthetic_pass: str, fmax_deferred, observer_gain: float
) -> None:
