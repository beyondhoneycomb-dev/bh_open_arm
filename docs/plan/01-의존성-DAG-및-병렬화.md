...
> 🔴 **총계는 171이 아니라 177이다.** 171은 `BOOT` 대역과 `WP-3A-00`이 들어오기 전의 수치이며, 이 문서가 DAG 노드 집합의 **총계 정본**이다: `171 + 1`(`WP-3A-00`, `02b` §3.5) `+ 5`(`WP-BOOT-01`~`05`, `06` §1.1) `= 177`. `00` §3.5 · `05` §0.1 · `06` §1.1이 `BOOT` 수용 게이트를 "171 WP 등재"라 적은 것은 **같은 회차에 신설된 5노드를 자기 자신이 세지 않은 것**이다 — 수용 게이트의 정확한 진술은 **"전 WP(177 = 비-`BOOT` 172 + `BOOT` 5) 등재 + `CI-01`~`CI-17` green"**이다. `06`의 양방향 대조 CI(§6.4)는 이 177을 기준으로 돈다.
...
| **`BOOT` 신설 5건** | **`WP-BOOT-01`~`05`** — 레지스트리 전량 + 매니페스트/인덱스 · CI-01~17 실물 · 상태저장소/취소 · 계약 해시 동결 잠금 | 계획의 집행체가 **존재하지 않았다.** 규칙과 규칙의 실행체를 같은 것으로 취급하면 전 불변식이 산문이다(`00` §3.5) |
...
|---|---|---|---|
| **🔴 −2 `BOOT` 실행 부트스트랩** | **계획을 실행할 기계** — 레지스트리 전량 · 매니페스트/인덱스 · CI-01~17 실물 · 상태저장소/취소 · 계약 동결 잠금 | **집행체**(전 배리어의 전제) | **전 WP(177) 등재 + `CI-01`~`CI-17` 자기 코퍼스 green.** 미달 = `FAIL_BLOCKING` → **어떤 WP도 착수 불가** |
...
**내부 미니 서브-DAG**: `BOOT-01` → `BOOT-02 · BOOT-03 · BOOT-04 · BOOT-05 (동시)`.
...
| `WP-BOOT-02` | **WP 매니페스트 스키마 + 인덱스 생성기**(`req_index`/`wp_index`/`glob_index`/`contract_index`/`gate_index`) | `AI-offline` | `SHAPE-IM` |
| `WP-BOOT-03` | 🔴 **CI 검사기 실물** — `CI-01`~`CI-17` 전량(`registry/check.py`) | `AI-offline` | `SHAPE-IM` |
| `WP-BOOT-04` | **워크플로우 상태 저장소 + 취소 메커니즘** — 실행클래스별 분기(`AI-offline` = 현 스텝 완주 / `AI-on-HW`·`Human-assisted-HW` = **즉시 스케줄러 latch-to-hold 후 취소**) | `AI-offline` | `SHAPE-IM` |
| `WP-BOOT-05` | **계약 해시 등재 + 동결 잠금**(`CONTRACT_FROZEN` 집행) | `AI-offline` | `SHAPE-CF` |

**동시성 근거**: `BOOT-01`(스키마 + 레지스트리 인스턴스)이 먼저 착지해야 나머지 넷이 질의·검증·잠금할 대상이 생긴다 — **코드 간선**. 그 뒤 `BOOT-02`~`05`는 서로 다른 트리를 배타 소유한다: `registry/generate/**` · `registry/checks/**` · `registry/state/**`+`ops/cancel/**` · `registry/contracts/**`. 공유 파일 0 · 공유 배타자원 0 · 물리 0(`AI-offline` — 로봇도 사람도 리그도 없다).
...
**목표**: 확인된 모순 4건(NORM-001~004)을 판정하고 승리 텍스트를 확정한다. **정규화 해시 참조 없는 WP 착수 금지.**
...
| `WP-N1-02` | 모순 4건 판정 적재 (NORM-001~004) | `AI-offline` | `SHAPE-CF` |
| `WP-N1-03` | 게이트 ID 네임스페이스 분리 · `M-8` 봉인 | `AI-offline` | `SHAPE-CF` |
| `WP-N1-04` | 정규화 해시 발행 · 착수 차단 게이트 | `AI-offline` | `SHAPE-IG` |

**동시성 근거**: `N1-02`(원장 4행 = `ledger/NORM-00n.md` 배타 소유)와 `N1-03`(게이트 네임스페이스 판정)은 서로 다른 원장 네임스페이스를 소유한다 — `N1-02` → `connect_contract`·`mode_contract`·`limit_contract`, `N1-03` → `gate_contract`. 공유 파일 0.
...
flowchart TD
  ...
  BOOTR["WP-BOOT-02..05<br/>매니페스트·인덱스 생성기 · CI-01~17 실물<br/>상태저장소+취소(latch-to-hold) · 계약 해시 동결잠금"]
