...
| Wave 0-B (`01`~`04` AI-offline/vcan · `05`~`08` AI-on-HW/실물) | `WP-0B-01`~`08` | 8 | `02a` §4.2·§4.3 |
...
> 🔴 **총계는 177이다.** 171은 `BOOT` 대역과 `WP-3A-00`이 들어오기 전의 수치이며, 이 문서가 DAG 노드 집합의 **총계 정본**이다: `171 + 1`(`WP-3A-00`, `02b` §5.1(근거 §5.0b)) `+ 5`(`WP-BOOT-01`~`05`, `06` §1.1) `= 177`. 수용 게이트의 정확한 진술은 **"전 WP(177 = 비-`BOOT` 172 + `BOOT` 5) 등재 + `CI-01`~`CI-17` green"**이며, `00` §3.5 · `05` §0.1 · `06` §1.1이 이 진술과 일치한다. `06`의 양방향 대조 CI(§6.4)는 이 177을 기준으로 돈다.
...
| **B-6 신설 1건** | **`WP-OPS-06`** — `CTR-ERR@v2`(`OA-*` 에러코드 레지스트리) 생산 WP | 전 대역이 소비하는데 생산 WP가 없어 퇴장 게이트가 공중에 떠 있었다 |
...
> 🔴 **이 표의 ID는 전부 `02a`/`02b`/`06`이 실제로 카탈로그·대역표에 실은 것이며, 이 문서가 발명한 것은 0건이다.** `WP-0C-09`(`02a` §5)·`WP-0A-04`(`02a` §3.2)·`WP-OPS-06`(`02a` §6)·`WP-2A-00`(`02b` §1.1)·`WP-3A-00`(`02b` §5.1 — `CTR-PRIM@v1` 소유)·`WP-BOOT-01`~`05`(`06` §1.1 — ID 발급처는 `02a`)를 직접 확인했다. 반대로 이 문서에 있고 카탈로그에 없는 ID는 오류이며, §6.4의 양방향 대조 검사가 잡는다.
...
| **0-Ops** | writer ACL·udev 영속·systemd CAN·버전핀·롤백·`push_to_hub=false`·구조화 로그·크래시 워치독 | 계약 | `CTR-ERR@v2` 동결 + 롤백 드릴 그린 |
...
| `WP-0A-01` | **`ActuationScheduler` 구현 (오프라인)** — 매 틱 4종 중 하나 방출 {수용 타깃 / stale-source 홀드 / 모드전환 홀드 / 안전래치 홀드}. 생산자는 타임스탬프 타깃을 **메일박스에 publish만**. 모드 전환 = 스케줄러 **정지 없이** 생산자 원자 교체 | `AI-offline` | `SHAPE-CF` | — |
...
| WP | 내용 | 클래스 | 형상 | 산출 게이트 |
|---|---|---|---|---|
| `WP-0B-01` | **flock 배타 락** — SocketCAN은 배타 bind를 제공하지 않는다 | `AI-offline` | `SHAPE-IM` | — |
...
| `WP-0C-09` | 🔴 **모드계층 + 드라이런 하드게이트** (B-2 이관: 구 `WP-2A-00`) — `SimBackend` 선택자(기본 MuJoCo 1단계), 드라이런 6검사기(위치·속도·토크·셀충돌·자가충돌·리프터), 실기전송 하드 차단 인터록, 검증 리포트 스키마 | `AI-offline` | `SHAPE-CF` → `SHAPE-IM(6)` | — |
...
| `WP-OPS-06` | **`OA-*` 에러코드 레지스트리 동결** — `OA-<도메인>-<3자리>`. **정본 = `14` §2.10** (B-6 신설) | `AI-offline` | `SHAPE-CF` | **`CTR-ERR@v2`** |

**동시성 근거**: 여섯 WP는 서로 다른 파일 트리를 소유한다(`/etc` 규칙·sandbox / 유닛 파일 / 락파일·롤백 / config 강제 주입 / 로깅·워치독 모듈 / 에러코드 레지스트리). 공유 import 0. `OPS-05`는 `OPS-06`이 동결한 코드를 **소비**하지만, `CTR-ERR@v2`이 대역 진입 전 동결되므로 C2를 만족한다.
...
**퇴장 게이트**: `CTR-ERR@v2` 동결 + 롤백 드릴 그린 + `push_to_hub` 기본값 재정의 실증.

> 🔴 **`CTR-ERR@v2`은 이제 생산자를 가진다 — `WP-OPS-06`**(B-6). 이 계약은 **전 대역이 소비**하는데(§6.2), 소비자만 있고 생산 WP가 없으면 퇴장 게이트가 공중에 뜬다. `OA-*` 레지스트리의 정본 텍스트는 `14` §2.10이고, `WP-OPS-06`은 그것을 **기계판독 레지스트리로 실체화**하는 노드다.
...
| ④ | `WP-1-04` | **읽기전용 측정** — 제어루프 사이클타임 히스토그램 + 사이클당 CAN 프레임 수(`candump -t d`: 8+8+8+8=32인가, 8+8=16인가). **부하 기준 = 합성 GIL 부하 하네스**(`WP-0C-06`) — 실카메라 조건 4는 3C의 `PG-RT-001b`가 소유 | `AI-offline` → `AI-on-HW` | `SHAPE-CF` → `SHAPE-MS` | **`PG-RT-001a`**(잠정, §2.5a) · **`PG-CAN-001`** |
...
| **인계** | `WP-1-02` → `WP-1-03` | `CTR-CAL@v2` 동결이 인계 조건. 영점 계약이 존재해야 홀드 타깃이 정의된다 |
...
| "②를 A, ③을 B에게 **동시** 배정" | ③의 상시 홀드는 **②가 확정한 영점 기준 자세**를 홀드 타깃으로 쓴다. `CTR-CAL@v2`이 ② 완료 전에는 존재하지 않는다 |
...
| WP | 계약 | 내용 | 산출 |
|---|---|---|---|
| **`WP-3A-00`** | 🔴 **공유 원시타입** | **3A의 단일 진입점.** 카메라 식별자(슬롯 키 문법·`left_`/`right_`·시뮬 namespace) · 타임스탬프 도메인(`CLOCK_MONOTONIC` ns · **만료 판정 시계 소유자 = 서버**) · 프레임 타입 태그(RGB/depth/채널) · 액션 페이로드 형상(position-only 8/16 + 단위 태그) · 큐 의미론(bounded·우선순위·drop·latest-wins) · 에러 봉투(`CTR-ERR@v2` 래핑) | **`CTR-PRIM@v1`** |
...
| `WP-3A-04` | **WS 봉투** | 단일 WS 멀티플렉싱(D-2) — **텍스트 = 텔레메트리/명령**, **바이너리 = 카메라ID+채널 태그**. 하트비트 · 스트림 통계 | `CTR-WS@v2` |
...
→ **`OaOpenArmFollower`의 동시점 소유자는 1명이다.** `WP-1-02`가 소유하고 `CTR-CAL@v2` 동결과 함께 `WP-1-03`에 **인계**한다. 순차 인계는 허용이며 동시 소유만 금지다 — `CTR-OWN@v1`이 이 소유권을 **`{파일, WP, 구간}` 3튜플**로 기계판독 가능하게 기록하고, CI가 **구간 겹침**을 차단한다.
...
| Wave 0-B (`0B-01`~`06`, `0B-08`) | 🔴 **계층별**(flat 7 아님): `0B-01·02` → `0B-03·05` → `0B-04·06` → `0B-08` (`0B-04` 이중바인드는 `0B-01`락·`0B-03`침입자 선행 — 02a 입력) | ✅ 각자 프로브 스크립트 | ✅ `CTR-OWN` 동결 | ✅ vcan 이름 분리 · USB↔CAN 분리 | §4.4 |
| Wave 0-B (`0B-07`) | 1 | ✅ | ✅ | ❌ **실물 CAN + 24V** | 배리어 뒤 |
| Wave 0-C (`0C-01`·`05`·`06`·`07`·`08`) | 🔴 **계층별**(flat 5 아님): `0C-05·06·07` → `0C-01·02·08` (`0C-08`은 `0C-07` 합성 데이터셋 소비 — 동시 아님, 02a 입력) | ✅ | ✅ `CTR-PLUG`·`CTR-ACT` 동결(0-A 퇴장) | ✅ | §4.5 |
...
| `CTR-PLUG@v1` | **통합 로봇플러그인 API**(`Robot` ABC) | `WP-0A-02` | 🔴 **Wave 0-A 퇴장**(= 첫 소비 웨이브 0-C 스폰 직전. 소유자 `WP-0A-02`가 0-A라 여기서 동결. `WP-1-01`은 이 계약을 **실기 축으로 소비·재확인**하지 동결하지 않는다 — 0-C가 미동결 계약을 소비하면 `CR-2` 위반) | `0C-01`,`0C-05`,`0C-09`,`1-02`,`5-11` | `09` §2.10 · `FR-SIM-097` · `01` `FR-SYS-014` |
| `CTR-ACT@v1` | **액션/관측 스키마**(`00` §8.3 6채널) | `WP-0A-02` | 🔴 **Wave 0-A 퇴장**(소유자 `WP-0A-02`가 0-A. `0C-07`/`0C-08`이 0-C에서 48채널 정의를 소비하므로 Wave 1 동결은 소비-선행-동결 위반 — CTR-PLUG와 동종) | `0C-07`,`0C-08`,`1-03`,`2A-*`,`3A-05`,`3B-11`,`4A-*`,`4B-02`,`WP-G-01`,`WP-G-02` | `10` `FR-TRN-066` · `07` `FR-REC-003`/`009`/`070` |
| `CTR-ERR@v2` | **`OA-*` 에러코드 레지스트리** | **`WP-OPS-06`** (B-6) | 🔴 **0-Ops 퇴장**(생산자 `WP-OPS-06`이 0-Ops 내부라 대역 진입 전 동결 불가 — 0-Ops 내부에서 `WP-OPS-06` → 소비자 순차, Wave 1 이전 완료) | **전부** | `14` §2.10 · `FR-OPS-020` |
| `CTR-CAL@v2` | **캘리브 JSON 스키마**(signs · 그리퍼 open/close rad · captured · timestamps · `zero_method` · `urdf_zero_offset` · `motor_zero_raw`) | `WP-1-02` | **`WP-1-03` 인계 시점** | `1-03`,`2A-08`,`2B-*`,`2D-05`,`3B-09`,`WP-G-S02` | `02` `FR-CON-031`/`063`/`064`/`065` · `16` `M-1` |
...
| `CTR-WS@v2` | **WS 봉투** | `WP-3A-04` | Wave 3B 진입 | **전 `WP-G-*`**,`3B-06`,`3B-15`,`3C-03`,`3D-08`,`5-05` | `13` §2.4 · `FR-GUI-040`/`041`/`048` · U-4 |
...
> **`진입(GUI WP) = max( CTR-WS@v2 동결(WP-3A-04) , 해당 도메인 백엔드 WP 착지 )`**
...
|---|---|---|---|---|---|
| `WP-G-S02` | **로봇 연결** | `/connection`,`/home-zero` | 3B | `WP-3A-04` · **`WP-1-02`**(`CTR-CAL@v2`) | — |
...
| 사실 | 함의 |
|---|---|
| `CTR-WS@v2`은 Wave 3A에서 동결된다 | **어떤 GUI WP도 Wave 3B 이전에 존재할 수 없다** |
...
flowchart TD
  ...
  subgraph W1["Wave 1 — 순차 서브-DAG (병렬 아님)"]
    ...
    W102["🔴 WP-1-02 ② OaOpenArmFollower 1차 소유<br/>connect_readonly → 명시영점 → 캘리브 영속<br/>CTR-CAL@v2"]
  ...
  W2A --> W3A0 --> B3A0A ==>|🔴 CTR-PRIM 미동결 시 3A-01~05 착수 불가| W3A
  ...
  A0C --> W5I
  C0A --> W5I
  %% W4B --> W5I 삭제: Isaac은 CTR-PLUG 후 어느 배치에도 붙는다(§5.4) — 4B 종속 아님. A0C/C0A 간선이 정본
```
