...
### 7.3 `CI-07` 판정 제외 해제 — 13건이 드러났고, 후속 세션에서 전부 판정됐다
...
사용자가 10개 항목에 답해 13건 전부 닫혔다. `registry.check --all` **GREEN**(판정 해시 보유
레코드 14 → 27). 원장 `NORM-008`~`NORM-014` 7행으로 적재했다 — `NORM-005`는 요구사항 모순이
아니라 참조 무결성 소견(`M-25` 인용이 실재하지 않음)이고 `WP-N1-03`이 게이트 매핑표에서
처리하도록 돼 있어 비워 뒀다.

| 원장 행 | 닫은 요구사항 | 판정 |
|---|---|---|
| `NORM-008` | `NFR-SAF-001`·`NFR-PRF-004`·`NFR-PRF-054`·`NFR-PRF-055`·`NFR-GUI-004` | 주파수는 게이트가 아니라 계측·표시. 단 감지 루프는 100 Hz |
| `NORM-009` | `FR-SAF-020` | 0.1×effort를 기본값으로. 운영자 변경·영속 필요, GUI가 "잔차 비교"임을 명시 |
| `NORM-010` | `FR-MAN-047`·`FR-GUI-118` | 홈 = `[0,0,0,π/2,…]`. 전 관절 0은 J4 하드스톱이라 폐기 |
| `NORM-011` | `NFR-SAF-002` | K=90. 운영자 변경·영속 필요 |
| `NORM-012` | `FR-SAF-031`·`FR-MAN-035` | **정책 변경** — v1 값을 기본 실행값으로, 재식별 경로 병행 |
| `NORM-013` | `NFR-REC-001` | 하한 없음, 보고만. 녹화 fps 조절 가능 |
| `NORM-014` | `NFR-GUI-008` | RT 승격 범위 제외 |

**판정 중 계산으로 잡은 모순 하나.** 사용자가 처음에 K=90 유지(결정 5)와 감지 루프 10 Hz
(결정 7)를 함께 답했는데, 이 둘은 양립 불가능하다. 관측기가 전진 오일러 적분이라
(`backend/gmo/observer.py:118`) `K·dt < 2`에서만 수렴하고, K=90에 10 Hz면 `K·dt = 9`다.
실행해 확인한 값:

| 루프 | K·dt | 외부 토크 5 N·m 계단 입력 후 잔차 |
|---|---|---|
| 100 Hz | 0.90 | 0 → 4.50 → 4.95 → **5.00** (수렴) |
| 20 Hz | 4.50 | 0 → 22.5 → −56.3 → 219 → −745 (발산) |
| 10 Hz | 9.00 | 0 → 45.0 → −315 → 2565 → **−20475** (발산) |

발산하면 임계값이 매 주기 넘어가 아무 일 없어도 계속 충돌로 뜬다 — 감지가 느려지는 게
아니라 **못 쓰게 된다.** 사용자 의도("1 kHz 같은 걸로 게이트 걸어 구현 늦추지 마라")는
게이트 제거로 그대로 지켜지므로, 감지 루프만 100 Hz로 두는 안을 제시해 채택됐다.
그 조건은 `NORM-008`의 `enforcement`에 `K·dt < 2` 런타임 검사로 걸어 뒀다.

**판정이 깨뜨린 테스트 2건 — 둘 다 "판정을 제대로 할수록 검사가 사라지는" 모양이었다.**

- `tests/n1/test_ledger.py` 가 원장 행 집합이 Wave −1 여섯 건과 **정확히 같음**을 단언했다.
  이후 판정은 정당한 추가이므로 **포함**으로 바꾸고 중복만 금지했다. 등호로 두면 앞으로의
  모든 판정이 깨진 테스트처럼 보인다.
- `tests/n1/test_seed.py::test_an_unsettled_contested_record_stays_null` 이 더 나빴다.
  "미판정 레코드는 null로 남는다"를 증명하려고 **실제 레지스트리에서 미판정 `결정필요` 레코드를
  찾아** 검사했는데, `assert open_records, "fixture needs a still-undecided requirement"` 가
  그 표본이 비면 실패한다. 즉 **마지막 논쟁 요구사항을 판정하는 순간 표본이 비어** 검사가
  성립하지 않는다. 표본이 비어도 통과하게 고쳤다면 그때부터는 아무것도 검사하지 않았을 것이다.
  파종기에 직접 묻도록 바꿨다 — 판정된 id는 스탬프를 받고 미판정 id(`FR-XXX-999`)는 None을
  받는지, 양방향을 못박는다. 빈 원장을 넣으면 실패하는 것까지 확인했다.

**판정에서 나온 구현 작업 6건**은 `docs/guide/04-판정에서-나온-구현-작업.md`에 정리했다.
전부 원장 `enforcement`에 `status: planned`로 걸려 있다. 코드에 아직 없는 것들이다 —
임계값·이득의 운영자 설정과 영속, GUI의 잔차 설명, 감지 루프 100 Hz, 주파수 게이트 제거,
녹화 fps 조절.

**남겨둔 위험**: `NORM-012`가 v1 마찰값을 기본으로 채택했는데, 잔차는 마찰 추정을 빼서
만들므로 마찰이 틀리면 잔차에 편향이 남아 `NORM-009`의 임계값이 어긋난다. 재식별 전까지
충돌 감지의 오검출·미검출은 이 편향을 먼저 의심한다 — 원장 `rationale`에도 적어 뒀다.
...
| `registry.check --all` | CI-01..CI-18, 35규칙 | **0 판정 finding — GREEN**(§7.5). 시작 시점 3,665건 |
...
1. ✅ **`CI-07` 13건 판정 완료** — §7.5. 원장 `NORM-008`~`NORM-014`, 게이트 전부 초록.
   다음은 그 판정이 요구하는 **구현 6건**(`docs/guide/04-판정에서-나온-구현-작업.md`)이다.
