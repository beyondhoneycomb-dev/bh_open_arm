...
| **그리퍼 — v2 pinch** | **pinch 그리퍼**: revolute **−45°–0°**(−0.7854..0 rad), 2지 mimic(finger2=finger1), **인핸드 카메라 내장.** 네이티브 제어 = **POS_FORCE**(`gripper_posforce_limits:[50.0, 1.0]` 리터럴). 🔴 **[개정 2026-07-15, H10] 2번째 값은 `torque_pu ∈ [0,1]`**(드라이버 내부 `/4.5`, `# TODO … Nm→pu?` 잔존)이지 **"1.0 Nm"이 아니다**; 속도 50은 **DM4310 vMax 30 rad/s 이하로 클램프**해야 한다(`03` FR-MOT-046/047, §5 Q-9). Nm/N 표시는 실기 파지력 캘리브레이션 후에만. 로드셀 없음 → open/close **엔드포인트 rad 캡처 → 디스크 영속**(참조 레포 패턴). v1의 88mm/링키지/파지력(N)은 **무효.** | `pinch_gripper/config/joint/joint_limits.yaml`·`joint_mimics.yaml`; `openarm_cell.yaml gripper_posforce`; 참조 레포 `capture_gripper_endpoint` |
...
> 🔴 웹 SPA에서는 데스크톱 판과 달리 **브라우저↔백엔드 경계가 실재**한다. 아래가 이 시스템의 **전 네트워크 경계**이며, 실시간 브라우저 채널은 **단일 WebSocket 하나**로 통일한다(D-2, 프로토콜 분리 금지). 포트는 설정 가능하고 기동 시 충돌을 감지한다(FR-GUI-006). 🔴 **[개정 2026-07-15, M8] 웹 백엔드 HTTP+WS 기본 포트는 `:8000`이며(SPA 서빙·REST·WS 공유), 전체 포트맵 정본(합집합)은 `01` §2.17 + `14` §2.1이다.**

| 경계 | 프로토콜/포트 | 방향 | 나르는 것 | 근거 |
|---|---|---|---|---|
| **브라우저 ↔ 백엔드 (실시간)** | **WebSocket**(HTTP(S) 위, 기본 SPA 서빙 포트 **:8000** 공유) | 양방향 | 텔레옵 명령 + 텔레메트리(관절/토크/상태/스트림통계) + **카메라 프레임(바이너리 JPEG/뎁스, 카메라ID+채널 태그)** | §2.1, §2.4; D-2; 포트맵 정본 `01` §2.17 + `14` §2.1 |
| **브라우저 ↔ 백엔드 (CRUD)** | **HTTP REST**(SPA 서빙 포트 **:8000**) | 요청/응답 | 설정 get/set, 데이터셋 목록/조회, 잡 시작/정지, 프로파일 — 비실시간. 표준 FastAPI, 프로토콜 분리 아님 | §2.1 |
| **브라우저 ↔ 백엔드 (정적)** | **HTTP(S) :8000** | 다운로드 | SPA 번들·폰트·URDF/메시·3D 자산(자체 호스팅, CDN 금지 — 에어갭) | FR-GUI-008 |
...
| FR-GUI-006 | 시스템은 **네트워크 경계가 있는 서비스 포트**(브라우저↔백엔드 HTTP/WS **:8000**, 원격 추론 gRPC 8080, VR UDP 5006, WebXR HTTPS 8443)를 설정 가능하게 하고, 기동 시 **포트 충돌을 감지·경고**해야 한다. | M | §2.7; 포트맵 정본 `01` §2.17 + `14` §2.1 | [개정 2026-07-15] |
...
| ID | 요구사항 | 우선 | 근거 | 비고 |
|---|---|---|---|---|
| FR-GUI-040 | GUI는 **단일 WebSocket 위에서 텔레메트리·명령·카메라 프레임을 멀티플렉싱**해야 한다(D-2 프로토콜 분리 금지). 카메라 프레임은 **바이너리 프레임**으로, **카메라ID + 채널 태그**(RGB/뎁스)를 헤더에 실어 다중 카메라를 구분한다. 텔레메트리(텍스트/소형 바이너리)와 카메라(대용량 바이너리)를 같은 WS에서 프레임 타입으로 분리한다. 🔴 **별도 WebRTC 스트림으로 분리하지 않는다.** 🔴 **[개정—검토권고 2026-07-15, H2] 단일 WS 멀티플렉싱은 HOL 블로킹이 내재하므로**(RFC6455 프레임 비인터리빙) 클래스별 bounded queue·텔레메트리 우선순위·`bufferedAmount` 차단·브라우저 worker 완화는 **FR-GUI-042가 강제**한다. | M | §2.1, §2.4; DECISIONS-v3 D-2 | **[신규구현] · [개정—검토권고 2026-07-15, H2]** — HOL 완화는 FR-GUI-042 참조 |
...
| FR-GUI-042 | 백엔드는 카메라 스트리밍이 **제어 루프를 굶기지 않도록** 프레임을 **스로틀·드롭(백프레셔)** 하고, 인코딩을 제어 루프와 **별도 워커**에서 수행해야 한다. WS 송신 버퍼가 임계를 넘으면 오래된 프레임을 버리고 **드롭 카운터를 상시 표시**한다. 🔴 카메라 인코딩 부하는 **NFR-GUI-011의 감시 대상**이다. 🔴 **[개정—검토권고 2026-07-15, H2] 단일 WS는 RFC6455상 프레임이 인터리빙되지 않으므로 큰 카메라 바이너리 프레임이 그 뒤의 텔레메트리를 막는 head-of-line(HOL) 블로킹이 구조적으로 존재한다.** 이를 완화하기 위해: **① 클래스별 bounded queue**(텔레메트리 큐와 카메라 큐를 분리·독립 상한), **② 텔레메트리 우선순위**(둘 다 대기 시 텔레메트리를 카메라보다 먼저 송신), **③ `bufferedAmount`(WS 송신 버퍼) 차단 임계**를 두어 임계 초과 시 카메라 프레임을 드롭하고 텔레메트리·명령은 보호, **④ 브라우저측 디코딩은 Web Worker**에서 수행해 렌더 메인스레드를 막지 않아야 한다. | M | §2.1 F-10′; NFR-GUI-011(제어 루프 지터); RFC6455 프레임 비인터리빙 → HOL 블로킹 불가피 | **[신규구현] · [개정—검토권고 2026-07-15, H2]** — HOL 완화(bounded queue·우선순위·bufferedAmount·worker) |
...
> **폐기된 읽기 (NORM-006 · NORM-007).** 아래 표의 요구사항이 한때 결정하던 문장들이고, 지금은
> 결정하지 않는다. 지운 문장을 여기 남기는 이유는 두 가지다 — 원장이 무엇을 폐기했는지 대조할
> 대상이 있어야 하고, 나중에 같은 문장이 표로 돌아오는 것이 되돌림이지 새 결정이 아니라는 것을
> 읽는 사람이 알아야 한다.
>
> - NORM-006, FR-GUI-065: `비상정지 컨트롤은 모든 화면·모든 모드·제어권 보유 여부와 무관하게 항상 접근 가능`
>   — 낱말 `비상정지`가 토크 유지와 전원 차단 둘 다를 가리켜, 주어가 정해지지 않은 채 능력을 부여했다. 주어는 STOP_HOLD다.
> - NORM-006, FR-GUI-067: `비상정지, 소프트 스톱` — 같은 낱말이 단축키 목록에서 소프트 스톱과 병렬로 놓여 하드를 뜻했다.
> - NORM-007, FR-GUI-063: `하드 E-Stop(전원 차단)을 서로 다른 컨트롤로 제공`
>   — §2.7 포트 매니페스트가 전 경계를 전수 열거하는데 릴레이·컨택터·GPIO·안전 PLC·E-Stop 접점이 0행이다.
> - NORM-007, FR-GUI-060: `E-Stop 상태` — 배지가 읽을 접점이 같은 이유로 없다.

| ID | 요구사항 | 우선 | 근거 | 비고 |
|---|---|---|---|---|
| FR-GUI-060 | GUI는 **로봇 상태 배지**를 전 화면 상단에 상시 표시해야 한다. 최소: **연결 상태**, **현재 모드**(LiveLinkMode, 모드 배지 = 상태전이), **활성 게인/리밋 프로파일명**, **제어권 보유자**. 전원라인 물리 버튼의 상태 배지는 두지 않는다 — §2.7 포트 매니페스트에 그 접점을 읽을 경계가 없으므로, 표시할 수 있는 것은 값이 아니라 값의 부재다. | M | NORM-007 판정(§2.7이 전 경계를 전수 열거하고 릴레이·컨택터·E-Stop 접점 0행) | [신규구현] |
...
| FR-GUI-063 | GUI는 **STOP_HOLD**(모터 토크로 자세 유지)를 **누를 수 있는 컨트롤**로 제공하고, **POWER_CUT**(전원 차단)은 컨트롤이 아니라 **지시·상태 패널**(전원라인 물리 버튼의 위치 안내 + 낙하 경고)로 제공해 둘의 시각 구분을 명확히 해야 한다. 소프트웨어가 전원을 끊는 경로는 두지 않는다. | M | OpenArm은 홀딩 브레이크가 없다 — 전원 차단 시 부하 급락. NORM-007 판정: 아무것도 하지 않는 버튼이 아무 버튼도 없는 것보다 나쁘다 | [확정] |
| FR-GUI-064 | GUI는 POWER_CUT 지시·상태 패널 안에 **"전원 차단 시 파지 중인 물체가 낙하한다"** 경고를 상시 표시해야 한다. | M | 공식 Safety Guide 인용 | [확정] |
| FR-GUI-065 | **STOP_HOLD 컨트롤은 모든 화면·모든 모드·제어권 보유 여부와 무관하게 항상 접근 가능**해야 한다. | M | `dora-openarm-evaluation-ui` "Keyboard shortcuts and an emergency stop"; NORM-006 판정(STOP_HOLD는 멱등·안전이므로 관찰자 포함 상시 가시) | [확정] |
| FR-GUI-066 | GUI는 **알림/경고 센터**를 제공해야 한다: 심각도(INFO/WARN/ERROR/CRITICAL), 소스, 타임스탬프, 상세, **ack 상태**. ERROR 이상은 ack 전까지 배지 유지. | M | — | [신규구현] |
| FR-GUI-067 | GUI는 **단축키**를 제공하고 매핑을 조회·재정의할 수 있어야 한다. 최소: STOP_HOLD, 에피소드 start/success/fail/cancel, 모드 전환, 3D 뷰 프리셋. **STOP_HOLD 키는 재정의 불가 예약 키**이고 관찰자에게도 노출된다. POWER_CUT 단축키는 두지 않는다 — 그 키가 누를 것이 없다. | M | `dora-openarm-evaluation-ui` "Keyboard shortcuts + emergency stop"; NORM-006 판정 | [확정] |
...
| FR-GUI-092 | 백엔드는 **제어권(command authority)을 한 시점에 한 WS 클라이언트에만** 부여하고, 나머지 접속 브라우저는 **관찰자(읽기전용, 텔레메트리·카메라만 수신)** 로 둔다. 제어권 이양은 **명시적 요청 + 현 보유자 통지**로 이루어지며, 관찰자는 `send_action` 경로가 서버측에서 거부되어야 한다. 🔴 **[개정—검토권고 2026-07-15, H3] 제어권 채널은 인증·역할 매핑 + WebSocket Origin allowlist + CSRF/CORS 방어**를 갖춰야 하며, WS는 **WSS/TLS** 위에서 서빙한다. 제어권은 **`{session_id, lease_generation, expiry, sequence, timestamp}` lease 계약**으로 부여·검증한다(만료·시퀀스 역행·중복 = **anti-replay 거부**). 🔴 **공개 health 엔드포인트는 제어권 보유자·활성 프로파일을 노출해서는 안 된다.** | M | 웹 다중 접속 가능 → 인증·anti-replay 없는 제어권 이양은 탈취 가능; 서버측 단일 제어권 중재 필요(F-1·F-2). `14` FR-OPS-074~080(lease·anti-replay)과 정합 | **[신규구현] · [개정—검토권고 2026-07-15, H3]** — 인증·anti-replay·lease·Origin allowlist 추가 |
| FR-GUI-093 | 🔴 **[개정—검토권고 2026-07-15, B4] 프로세스 사망(크래시/SIGKILL/OOM/deadlock)이나 CAN bus-off = 낙하다 — 소프트 홀드가 불가능하다.** 제어 프로세스가 죽으면 `STOP_HOLD` 명령을 보낼 주체가 사라지고, DM 모터의 통신두절 워치독(通讯丢失防护)이 설정 주기 내 CAN 미수신 시 **자동으로 인에이블을 이탈**시켜 토크가 0이 되고 **팔이 낙하한다**(기계식 브레이크 없음, `12` §2.3·§2.10 matrix·NFR-SAF-009, `14` FR-OPS-024). 따라서 **"마지막 셋포인트가 MIT 게인으로 유지되어 프로세스가 죽어도 안 멈춘다"는 전제는 틀렸다** — 모터 워치독 타임아웃 뒤에는 홀드가 소멸한다. **소프트웨어 워치독은 낙하를 늦출 뿐 막지 못하며**, 프로세스 사망·bus-off의 실패안전은 **기계적 지지·낙하영역 격리·독립 전원차단 회로**로만 성립한다(외부 안전 컨트롤러/독립 CAN takeover 없는 한). **살아 있는** 제어 루프의 하트비트 소실은 즉시 `STOP_HOLD`로 전이하되(이는 프로세스가 아직 살아 명령을 보낼 수 있을 때만 유효), **WS 클라이언트 이탈은 소프트 스톱 사유가 아니다**(제어권은 서버가 쥔다) — 단 제어권 보유 클라이언트의 하트비트 소실은 `STOP_HOLD`로 전이한다. | M | 통신두절 워치독 = 낙하(`12` §2.3·NFR-SAF-009); `14` FR-OPS-024(낙하 규정)와 정합 — v1의 "크래시 후 마지막 명령 유지" 오해 정정 | **[개정—검토권고 2026-07-15, B4]** — FR-OPS-024와의 모순 해소 |
...
> **폐기된 읽기 (NORM-007).** FR-GUI-111이 한때 `충돌 반응 정책(소프트 스톱 / 속도 스케일 하향 / 하드 E-Stop)`
> 으로 열거했다. 마지막 항목은 소프트웨어가 전원을 끊는 경로를 전제하는데 §2.7에 그 경계가 없다.
> 고를 수 있게 두면 고를 수 없는 것을 고르게 하는 것이므로 열거에서 뺐다.
...
| FR-GUI-108 | **S-03 모터 설정**은 CAN ID 맵(send `0x01–0x08` / recv `0x11–0x18`), 모터 타입, **게인/리밋 프로파일 편집·전환·검증**(compliant/stiff, `kp∈[0,500]`/`kd∈[0,5]` 검증), 모터별 **온도(T_MOS/T_Rotor)** 를 표시해야 한다. 온도는 상태 프레임에 이미 실려 오므로 **별도 폴링 UI를 만들지 않아야** 한다. **그리퍼는 POS_FORCE + open/close 엔드포인트 rad 캡처**로 다룬다 — 🔴 **[개정 2026-07-15, H10] POS_FORCE 2번째 값은 `torque_pu∈[0,1]`이지 Nm이 아니며, 속도는 DM4310 vMax 30 rad/s로 클램프**한다. 파지력을 N/Nm으로 표시하지 않는다(pu↔N 미확정, §5 Q-9, `03` FR-MOT-046/047). | M | `dm_motor_control.cpp::parse_motor_state_data`(온도); `12` FR-SAF-062(게인 검증); `03`(MOT)이 그리퍼 소유 | [개정 2026-07-15] |
...
| FR-GUI-111 | **S-12 충돌·안전**은 가상벽/금지영역을 3D 뷰포트에서 **직접 편집**하고 충돌 반응 정책(소프트 스톱 / 속도 스케일 하향)을 선택할 수 있어야 하며, **충돌 감지의 기본 반응이 전원 차단이어서는 안 된다.** 전원 차단은 선택지 자체가 아니다 — §2.7에 그 경계가 없으므로 고를 수 있게 두면 고를 수 없는 것을 고르게 하는 것이다. | M | 홀딩 브레이크 부재 → 전원 차단 = 낙하(F-8) | [확정] |
...
> **폐기된 읽기 (NORM-006).** NFR-GUI-005가 한때 `비상정지 명령의 왕복 지연 상한(브라우저 입력 → WS → 백엔드 STOP_HOLD 적용)`
> 으로 적혀 있었다. 괄호 안은 STOP_HOLD를 말하는데 낱말은 `비상정지`여서, 같은 문서의 단축키
> 목록이 같은 낱말로 하드를 가리키는 것과 충돌했다. 주어를 STOP_HOLD로 못박았다.
...
| NFR-GUI-005 | STOP_HOLD 명령의 **왕복 지연 상한**(브라우저 입력 → WS → 백엔드 `STOP_HOLD` 적용)을 정의하고 초과 시 경고해야 한다. **수치는 자체 벤치로 확정한다.** (텔레옵 지연 근거로 인용되던 arXiv 2603.06850은 CARLA 차량 차선유지 실험이라 도메인이 다르다 — 그 수치를 상한으로 쓰지 않는다.) | M | 도메인 불일치; WS 홉 포함 | [결정필요] |
...
| NFR-GUI-011 | 🔴 GUI는 **LeRobot 제어 루프(백엔드)의 사이클 타임(p50/p95/p99)** 을 상시 계측·표시하고, 목표 fps 대비 0.95배 미만이 N사이클 연속이면 WARN + 원인 구간(CAN / 카메라 grab·인코딩 / IK / WS 직렬화) 분해 표시해야 한다. 🔴 **[개정 2026-07-15, H4] "단일 논리 서비스"와 "단일 OS 프로세스"를 구분한다.** 프로세스 토폴로지 정본은 **`15` NFR-PRF-038**이다: **CAN을 소유한 제어 루프(RT 승격 스레드) + 안전 루프만 단일 CAN-owner 프로세스에 두고**, GIL을 다투는 무거운 CPU 작업(**카메라 JPEG 인코딩·트랜스코딩·데이터셋 쓰기·GUI 직렬화·추론**)은 **별도 프로세스**로 분리한다(Python 스레드는 GIL 때문에 인코딩을 참 병렬화하지 못해 제어 루프 지터를 만든다). 분리 시 **IPC·공유메모리 수명·worker watchdog·CAN write owner 단일화**를 정의해야 하며 `connect()` 1회 원칙(F-3′)은 유지한다(제어 프로세스가 `Robot`을 독점). | M | 🔴 한 프로세스에 CAN(16모터)+카메라 grab·JPEG 인코딩+IK(0.345 ms)+WS 서빙이 얹힌다. **[미측정]** — `09` NFR-SIM-007 · `12` FR-SAF-001b와 **같은 측정 하나**로 해결; 프로세스 토폴로지 정본 = `15` NFR-PRF-038 | **[신규구현] · [개정 2026-07-15, H4]** — 논리서비스/OS프로세스 구분 + 무거운 작업 별도 프로세스(15 정본) |
...
| NFR-GUI-012 | WS 링크 대역폭 예산을 정의하고 상시 계측해야 한다: 카메라 스트림 합계(JPEG bytes/s) + 텔레메트리. 링크 포화 시 **카메라 품질/fps를 우선 낮추고 제어·텔레메트리를 보호**해야 한다. LAN vs 로컬호스트 클라이언트 여부를 표시한다. 🔴 **[개정—검토권고 2026-07-15, H2] 부하시험 합격조건을 정의해야 한다**: 최대 카메라 스트림 부하(다중 카메라·최대 해상도·다중 클라이언트)에서 **텔레메트리·명령의 p99 왕복 지연이 임계 이하**로 유지되는지(HOL 블로킹으로 제어 채널이 밀리지 않는지)를 측정해 합격/불합격을 판정한다. | S | 웹 스트리밍의 실재 비용(§2.1, FR-GUI-042); RFC6455 HOL 블로킹 | **[신규구현] · [개정—검토권고 2026-07-15, H2]** — p99 지연 부하시험 합격조건 추가 |
...
| Q-14 | 🔴 **제어 루프가 백엔드 프로세스 안에 있다. 카메라 인코딩·WS 서빙이 제어 루프를 방해하는가?** (NFR-GUI-008 / NFR-GUI-011) LeRobot `record_loop()`가 백엔드 제어 루프에서 돌고, 같은 프로세스가 카메라를 JPEG 인코딩해 WS로 밀며 텔레메트리를 직렬화한다. **LeRobot 루프 사이클 타임 [미측정].** | **측정하라.** ① 카메라 스트림 개수·해상도·JPEG 품질별로 제어 루프 사이클 타임 p50/p95/p99 비교, ② 인코딩·WS 서빙을 별 워커/스레드로 격리한 효과 분리, ③ 클라이언트 접속 수(다중 브라우저)에 따른 부하. 🔴 **[개정 2026-07-15, H4] 정본 프로세스 토폴로지(`15` NFR-PRF-038)는 무거운 CPU 작업(카메라 JPEG 인코딩·트랜스코딩·데이터셋 쓰기·GUI 직렬화·추론)을 이미 별도 프로세스로 규정한다** — GIL 때문에 스레드 격리로는 참 병렬이 안 되므로, "대안이 없으면"이 아니라 **기본이 별 프로세스**다. 상태를 IPC(공유메모리)로 넘기되 `connect()` 1회 원칙(F-3′)은 유지 — 제어 프로세스가 `Robot`을 독점하고 CAN write owner는 하나다. 측정은 그 분리 효과의 정량화이지 분리 여부의 판단이 아니다. 🔴 **`09` Q13 · `12` Q17과 동일한 실험 하나. 먼저 재라.** | **[결정필요] — 백엔드 아키텍처 최대 미결. [개정 2026-07-15, H4] 무거운 작업 별도 프로세스가 정본(15 NFR-PRF-038)** |
...
**🔴 LeRobot — 정본 런타임 (v0.6.0 공개 최신; commit SHA로 핀 권장. 직접 열람). §2.2의 F-3′ ~ F-7′ 근거** — [개정 2026-07-15] `v0.6.1`은 존재하지 않는 유령 버전이라 공개 최신 `v0.6.0`(2026-07-06)으로 정정.
